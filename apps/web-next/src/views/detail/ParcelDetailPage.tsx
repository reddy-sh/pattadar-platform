...
export function ParcelDetailPage() {
        ...
        <SectionCard title="Parcel record">
          <FieldGrid>
            ...
            <Field label="Pattadar passbook">
              {data.passbookId ? (
                <Link component={RouterLink} href={`/app/passbooks/${data.passbookId}`}>
                  {p.passbookRef}
                </Link>
              ) : (
      ...
      {data.groupName && (
        <Box sx={{ mt: 1.5 }}>
          <SectionCard
            ...
            action={
              <Link component={RouterLink} href="/app/groups" variant="caption">
                Open group ›
              </Link>
            }
          >
  ...
  return (
    <>
      ...
      <Box sx={{ mt: 2 }}>
        <Link component="button" variant="body2" onClick={() => router.push('/app/parcels')}>
