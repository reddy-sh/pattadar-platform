// Must run before amazon-cognito-identity-js loads (needs a Node-style `global`).
 ...
 *
 * Pool config comes from NEXT_PUBLIC_COGNITO_AUTHORITY (the pool id is the
 * tail of the issuer URL) + NEXT_PUBLIC_COGNITO_CLIENT_ID. In mock mode
 * (authority unset) every call rejects with NOT_CONFIGURED — AuthProvider
 * never calls in here when mocked.
 */
import {
