 ...
 *
 * Ported from apps/web/src/pages/auth/ForgotPasswordPage.tsx. Deviation: the
 * success notice travels to /login as a `?notice=` query param instead of
 * react-router location state (no App Router equivalent).
 */
...
import { AuthError, confirmForgotPassword, forgotPassword } from 'src/auth/cognitoNative';
...
export default function ForgotPasswordPage() {
  ...
  const onReset = async (e: FormEvent) => {
    ...
    try {
      ...
      router.replace(
        `/login?notice=${encodeURIComponent('Your password has been changed. Sign in with the new password.')}`,
      );
    } catch (err) {
