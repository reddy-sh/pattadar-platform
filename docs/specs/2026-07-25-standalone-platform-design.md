...
| D4 | **Keep Auth0** (free tier to 25K MAU; identity format preserved). Cognito documented as a later cost lever only | Recommended — confirm |
...
| **1** | Backend lift: api image → ECS; `pg_dump` → RDS; slim gateway (auth + storage-on-S3 + proxy); MinIO → S3 mirror (final incremental sync during write freeze); EventBridge cron | On AWS with a real Auth0 token: GraphQL round-trip, one AI extraction, one upload + preview, cron fires with secret |
| **2** | MUI web app view-by-view; extract `packages/core` as views are rebuilt; Auth0 SPA integration | Per-view parity checklist vs rhub pattadar |
...
- **Security:** KMS CMK everywhere at rest; TLS-only; Auth0 MFA; IAM least-privilege with OIDC-federated CI (no long-lived keys); fail-closed admin gates; GuardDuty malware scanning gating document availability.
- **Privacy (GDPR/DPDP):** ROPA for pattadar's data classes (identity incl. masked Aadhaar, documents, minors/guardians); DSR work items — export endpoint, erasure cascade (DB rows + storage nodes + S3 versions + Auth0 user); retention schedule; cross-border processors documented (Auth0, Anthropic — DPDP-permitted; Bedrock Mumbai as future in-region option); breach runbook (GDPR 72 h / DPDP Board); verifiable parental consent for the existing minor→guardian flow.
