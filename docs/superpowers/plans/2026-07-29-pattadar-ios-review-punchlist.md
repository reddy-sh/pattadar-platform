...
**Verification model (corrected):** the repo DOES have tests — self‑contained Bun guard scripts under `scripts/*-tests.ts` (e.g. `bun run scripts/contrast-tests.ts`, `area-tests.ts`, `geo-tests.ts`, `village-tests.ts`, `plural-tests.ts`, `screening-tests.ts`, `vocab-tests.ts`, `ux-guards.ts`, `icon-guard.ts` — several read `apps/mobile/src/theme/paper.ts`/source as text and assert), plus Maestro flows in `tests/e2e-mobile/*.yaml` (device/simulator only). Each task verifies with: (1) **`bun run typecheck`** clean; (2) the **relevant guard script(s)** green — notably `contrast-tests.ts` for any `paper.ts`/color change (Tasks 4.1/4.4/5.1), `icon-guard.ts` for Task 0.3, `area-tests.ts` for unit conversion, and `ux-guards.ts` (may already enforce the no‑dev‑copy rule in Task 5.3 — run it and extend it, don't fight it); (3) an explicit manual/device check named in the task. New pure‑logic guards (e.g. a contrast helper, the hook's error branch) follow the existing `scripts/*-tests.ts` pattern — a Bun script that reads source as text — NOT a react‑native import (that doesn't parse outside the bundler).
...
- [ ] **Verify:** `cd apps/mobile && npx expo config --type introspect | grep -iE "NSLocationWhenInUse|NSFaceID"` shows both strings; `bun run typecheck` clean.
...
```
- [ ] **Verify:** `bun run typecheck` clean; on device, a screen using `useSafeAreaInsets` (e.g. `set-location.tsx`) reports non-zero top inset on a notched device.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, after sign-out, `SecureStore.getItemAsync('pattadar_tokens')` returns null and a subsequent storage call fails auth (StorageAuthError) instead of succeeding.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, a cached file older than TTL is removed on next open; "Clear cached files" empties both dirs; new local copies are excluded from backup.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, after delete, no `doc-<id>-*` or `drive-<nodeId>-*` file remains.
...
- [ ] **Verify:** `bun run typecheck` clean; saving many documents persists all references; a simulated write failure is surfaced, not swallowed.
...
- [ ] **Verify:** `bun run typecheck` clean; a PDF still renders; an HTML file with an inline `<script>` that reads another cached file does nothing.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, each Aadhaar reveal prompts Face ID; entering an `http://` override in a release build is rejected with an error.
...
- [ ] **Verify:** `bun run typecheck` clean; storage still uploads/downloads (regression check).
...
- [ ] **Verify:** `bun run typecheck` clean; against a gateway build, a GraphQL query and a passbook import both succeed with Bearer and no `x-user-id`; a request with a tampered/absent token is rejected by the gateway (401).
...
**Files:** Modify `apps/mobile/src/data/hooks.ts:109-139`. Extract the failure-merge into a pure helper (e.g. `mergeOnFailure(prev, empty)`) so it's testable via a Bun script `scripts/hooks-tests.ts` following the `scripts/*-tests.ts` pattern (no react-query/react import — test the pure function).

- [ ] **Write the failing Bun test** (`scripts/hooks-tests.ts`): `mergeOnFailure(prev, empty)` returns `{data: prev.data, isSample:true}` when `prev` exists; `{data: empty(), isSample:true}` when `prev` is undefined. Run `bun run scripts/hooks-tests.ts` → fails.
- [ ] Implement the helper + use it in the queryFn's final `catch`: `const prev = queryClient.getQueryData<LiveResult<T>>(['pattadar', key]); return mergeOnFailure(prev, empty);` so a transient outage never wipes a landowner's already-loaded holdings.
- [ ] Run `bun run scripts/hooks-tests.ts` → passes.
- [ ] **Verify:** `bun run typecheck` clean; the Bun test green.
...
- [ ] **Verify:** `bun run typecheck` clean; with the server unreachable, each list screen shows retry (not "you own nothing"); after a real empty account, the normal empty CTA still shows.
...
- [ ] **Verify:** `bun run typecheck` clean; VoiceOver announces the person's name; initials are legible on a light generated background.
...
- [ ] **Verify:** `bun run typecheck` clean; VoiceOver announces selected option state; a long picker scrolls; the sheet traps focus and doesn't announce phantom buttons.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, each control has a ≥44pt tappable area (Accessibility Inspector).
...
- [ ] **Verify:** `bun run typecheck` clean; at XXL Dynamic Type, rows grow without clipping.
...
- [ ] **Verify:** `bun run typecheck` clean; on a home-indicator device, Save/Cancel bars clear the indicator.
...
- [ ] **Verify:** `bun run typecheck` clean; greens/ambers adapt correctly in dark mode.
...
- [ ] **Verify:** `bun run typecheck` clean; icons render; VoiceOver no longer reads an emoji name.
...
- [ ] **Verify:** `bun run typecheck` clean; no dead rows or dev copy in a release build.
...
- [ ] **Verify:** `bun run typecheck` clean; grabber adapts to theme; VoiceOver announces each cell as a labelled button.
...
- [ ] **Verify:** `bun run typecheck` clean; settings render as grouped rounded cards on device.
...
- [ ] **Verify:** `bun run typecheck` clean; on a property, "Move to village" + "Clear location" actually update the pin; ownership change shows a confirm summary.
...
- [ ] **Verify:** `bun run typecheck` clean; scanning a house doc selects "Independent house".
...
- [ ] **Verify:** `bun run typecheck` clean; with consent off, the member is saved without a photo.
...
- [ ] **Verify:** `bun run typecheck` clean; row tap goes one place; VoiceOver announces no phantom buttons.
...
- [ ] **Verify:** `bun run typecheck` clean; removing an heir requires a deliberate, non-adjacent action.
...
- [ ] **Verify:** `bun run typecheck` clean; a portrait scan fills width; VoiceOver labels the image; local files don't say "Fetching from My Drive".
...
- [ ] **Verify:** `bun run typecheck` clean; the local notification body contains no record details.
...
- [ ] **Verify:** `bun run typecheck` clean; VoiceOver announces the address as the pin moves; results are distinguishable.
...
- [ ] **Verify:** `bun run typecheck` clean; add vs delete is distinguishable in grayscale / to VoiceOver.
...
- [ ] **Verify:** `bun run typecheck` clean; a large upload shows percent + cancel; no temp files linger; permission denial offers Settings.
...
- [ ] **Verify:** `bun run typecheck` clean; on device, it enumerates the signed-in user's drive files.
...
- [ ] **Verify:** `bun run typecheck` clean; export produces an archive containing the manifest + every document; opening it shows all records.
...
- [ ] **Verify:** `bun run typecheck` clean; exporting then restoring into a clean account reproduces the records; Delete-account performs a real action, not an email draft.
