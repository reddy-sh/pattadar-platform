...
Currently seeded with DD/MM/YYYY date helpers (`src/format/date.ts`) and a few domain types (`src/types/index.ts`). The real content arrives in Phase 2 by porting the modules below out of rhub.

## PORT MAP — rhub → core

Source root: `/Users/reddy.sh/reddy.sh/projects/rhub/ui/apps/pattadar/src/`

| rhub source | Future home | What it is | Tests in rhub |
| --- | --- | --- | --- |
...
| `exporters.ts` (data preparation only; file now lives at `ui/platform/design-system/src/exporters.ts` in rhub) | `core/export` | Row/column shaping for exports. The jspdf/xlsx rendering itself stays web-only in apps/web | No |
...
- `landcalc`, `units`, `dashboard`, and `docTypes` are already cleanly separated pure modules with unit tests in rhub — these port first and nearly verbatim (their tests come along).
